package com.flchy.cloud.service;


import com.google.common.collect.Maps;
import com.flchy.cloud.enums.BaseEWarning;
import com.flchy.cloud.enums.Executable;
...
public class TransactionOptDelayer {
    ...
    private static ThreadLocal<Map<String, Stack<Executable>>> afterTransactionOpts = new NamedThreadLocal<>(
            "redis后置操作thread-local");

    /**
     * 延迟执行代码,直至事务提交 如果当前不在事务中,抛出异常
     *
     * @param executable
     */
    public void executeAfterTransactionCommit(Executable executable) {
        if (TransactionSynchronizationManager.isActualTransactionActive()) {
            ...
            BusinessException e = new BusinessException(BaseEWarning.SERVER_ERROR, "事务后置操作*必须*在一个活跃的事务中");
    ...
    /**
     * spring事务默认为全类名加方法名,如果是编程事务,可能会null 为null的情况下,使用default
     *
     * @return
     */
    ...
    private static class AfterTransactionSynchronization extends TransactionSynchronizationAdapter {
        ...
        /**
         * 事务提交成功后,执行操作 如果事务没有成功提交,绑定的操作会在afterCompletion方法中清除
         */
        @Override
